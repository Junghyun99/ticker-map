...
import os
import shutil
from src.utils.ticker_reader import get_ticker_info, get_alias, get_exchange, DEFAULT_DB_PATH
...
def test_get_alias_success():
    alias = get_alias("005930")
    assert alias == "삼성전자"

def test_get_exchange_success():
    exchange = get_exchange("005930")
    assert exchange == "KS"

def test_helpers_not_found():
    assert get_alias("NON_EXISTENT") is None
    assert get_exchange("NON_EXISTENT") is None
