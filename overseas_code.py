...
def get_overseas_master_dataframe(base_dir,val):
    ...
    urllib.request.urlretrieve(f"https://new.real.download.dws.co.kr/common/master/{val}mst.cod.zip", zip_file_path)
    
    os.chdir(base_dir)

    overseas_zip = zipfile.ZipFile(f'{val}mst.cod.zip')
    extracted_files = overseas_zip.namelist()
    overseas_zip.extractall()
    overseas_zip.close()

    # 압축 해제된 실제 파일명을 동적으로 찾기 (대소문자 상관없이)
    file_name = os.path.join(base_dir, extracted_files[0])
    ...
    df.columns = columns
    xlsx_file = f'{val}_code.xlsx'
    df.to_excel(xlsx_file, index=False)  # 현재 위치에 엑셀파일로 저장
